...
class BaseTank(BaseItem):
    ...
    def __init__(self, left, top):
        ...
        # 存放图片的字典
        self.images = {
            'U': pygame.image.load('tank_img/p4.png'),
            'D': pygame.image.load('tank_img/p4.png'),
            'L': pygame.image.load('tank_img/p4.png'),
            'R': pygame.image.load('tank_img/p4.png')
        }
...
class EnemyTank(BaseTank):
    def __init__(self, left, top, speed):
        ...
        self.images = {
            'U': pygame.image.load('tank_img/enemy3.png'),
            'D': pygame.image.load('tank_img/enemy3.png'),
            'L': pygame.image.load('tank_img/enemy3.png'),
            'R': pygame.image.load('tank_img/enemy3.png')
        }
...
class enemy_gun(EnemyTank):
    def __init__(self,left, top, speed):
        super().__init__(left,top,speed)
        #弓兵射击
    def shot_strength(self):
        num = random.randint(1, 100)
        if num < 4:
            return Bullet(self)
class enemy_bubing(EnemyTank):
    def __init__(self,left,top,speed):
        super().__init__(left,top,speed)
    def enemyTank_hit_MyTank(self):
        for enemy in MainGame.EnemyTankList:
            if MainGame.my_tank and MainGame.my_tank.live:
                if pygame.sprite.collide_rect(MainGame.my_tank, enemy):
                    self.stay()
    def enemyTank_hit_MyTank(self):
        for enemy in MainGame.EnemyBulletList:
            if MainGame.my_tank and MainGame.my_tank.live:
                if pygame.sprite.collide_rect(MainGame.my_tank, enemy):
                    self.stay()
...
class Wall:
    def __init__(self, left, top):
        self.image = pygame.image.load('tank_img/house.png')
        self.rect = self.image.get_rect()
        self.rect.left = left
        self.rect.top = top
        self.live = True

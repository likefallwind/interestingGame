...
def randomAI():
    next_point = np.random.randint(COLUMN, size = 2)
    next_point = list(next_point)
    while next_point in list3:
        next_point = np.random.randint(COLUMN, size = 2)
        next_point = list(next_point)
    return next_point[0], next_point[1]
        ...
        for n in range(ROW):
            ...
            if n < ROW - 4 and (m, n) in list and (m, n + 1) in list and (m, n + 2) in list and (
                ...
                return True
                        ...
                        m + 2, n + 2) in list and (m + 3, n + 3) in list:
                return True
            elif m < ROW - 4 and n > 3 and (m, n) in list and (m + 1, n - 1) in list and (
                        m + 2, n - 2) in list and (m + 3, n - 3) in list:
                return True
...
def main():
    ...
    while g == 0:
        ...
        if change % 2 == 1:
            ...
            if game_win(list1):
                message = Text(Point(100, 100), "白子获胜")
                message.draw(win)
                g = 1
            change = change + 1
            ...
            if not ((round((p2.getX()) / GRID_WIDTH), round((p2.getY()) / GRID_WIDTH)) in list3):
                ...
                piece.draw(win)
                if game_win(list2):
                    message = Text(Point(100, 100), "黑子获胜")
                    message.draw(win)
                    g = 1
